...
- **星座守护星**：分析守护星对性格和运势的影响
- **星座宫位特征**：基本宫（开创性）、固定宫（稳定性）、变动宫（适应性）对运势的不同影响
- **健康倾向**：每个星座容易出现的健康问题和保健重点
...
1. **整体运势评级**（3-5星，必须至少3星，尽量不要是4星，3、4、5星需要随机选择）
2. **健康运势**（详细分析，约100-150字，要求高度多元化，主要基于姓名五行属性和星座健康倾向，可选择以下角度组合分析：
   - 身体层面：根据姓名五行和星座分析体质强弱、免疫力、慢性疾病预防、身体机能、精力状态、脏腑调理
...
3. **健康建议**（具体建议，约50-80字，要求实用多样，主要基于姓名特征和星座属性，可选择以下方面组合：
   - 饮食方面：根据姓名五行和星座属性推荐营养搭配、食疗建议、饮食禁忌、进餐时间、食材选择
...
6. **幸运色**（格式：颜色名称 (Hex格式)，要求使用丰富多样的颜色：
   
   传统中华颜色库参考（需要随机选择，避免重复）：
   红色系：朱砂红(#FF4500)、胭脂红(#C93756)、枣红(#C32136)、樱桃红(#DE1C31)、玫瑰红(#F47983)、茜草红(#CB3A56)、银红(#F05654)、妃红(#ED5736)、海棠红(#DB5A6B)、石榴红(#F20C00)
   橙色系：橘红(#FF7500)、柿子橙(#FF6600)、杏黄(#FFA500)、橙黄(#FF8C00)、蜜桃橙(#FFCC5C)、夕阳橙(#FF7F00)、桔梗橙(#FF8936)
...
重要要求：
- **分析融合**：以姓名深度解析为主导（50%），十二星座特征为重要辅助（30%），环境因素为调节（20%），三者有机融合，自然渗透到各项运势分析中
...
- **内容多样**：生成时从随机的维度组合分析角度，确保同一用户多次查询也能获得丰富多样的内容
- **个性化程度**：通过姓名和星座的深度结合，让每个用户的运势分析都具有高度个性化特征
- **语言风格**：温暖、积极、专业，体现东方文化智慧，避免过于神秘或西化表达
- **评分范围**：整体运势3-5星，3星、4星、5星完全随机
- 语言要温暖、关怀，适合年长用户阅读
- 内容要积极正面，体现东方文化特色
- **幸运色多样性**：必须从丰富的颜色库中选择，选择需要完全随机
- 健康建议要实用、温和，必须结合当前天气条件和星座特征给出针对性建议
- 行动建议必须考虑当前所在地的地理位置、天气状况和星座特性，字数要达到50-80字，但是禁止出现地名
...
请以JSON格式返回，结构如下：
{
  ...
  "actionSuggestion": "行动建议，以姓名特征、能量周期、性格映射为主要依据，禁止出现地名深度结合星座行为模式、习性偏好和地理天气条件，具体可行..."
}

注意：overallRating必须是3、4或5这三个数字中的一个，不能是字符串。请严格按照分析优先级：以姓名多维度特征为主导（占50%权重），星座深度特征为重要辅助（占30%权重），环境因素为调节（占20%权重）进行综合评定。所有输出内容要自然流畅，通过姓名和星座的深度融合分析，避免直接提及具体分析方法或依据，让用户感受到基于深层个人特质和天赋禀性的专业运势指导，确保每次生成的内容都具有高度个性化和多样性。`
}
